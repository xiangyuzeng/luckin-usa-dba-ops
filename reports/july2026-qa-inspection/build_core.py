...
# -*- coding: utf-8 -*-
"""
July 2026 QA store-inspection — CORE build.
Reuses the proven May/June extraction logic (tables/joins/score-formula/appeal-semantics)
with the July window, and emits the enhanced CSV schema + derived.json
...
EN2CN = {
    ...
    "Site Security": "职业安全",          # resolved 2026-07-01, carried forward
    "Pests Control": "虫害防控",
...
H   = L("july_headers.json")
REP = L("july_reports.json")
OPP = L("july_opps.json")
APP = L("july_appeals.json")
STO = L("july_stores.json")
JANMAY_H  = L("janmay_headers.json")      # Jan-May headers (all statuses)
APRMAY_R  = L("aprmay_reports.json")      # Apr-May reports
JUNE_H    = L("june_headers.json")        # June headers (submitted)
JUNE_R    = L("june_reports_hist.json")   # June reports
...
# ---------------------------------------------------------------- 主巡检 derivation
ACTIVE_STORES = {}     # store_code -> store row (status=1, non-test, opened)
for s in STO:
...
OPENED_NOT_INSPECTED.sort(key=lambda r: r["store_code"])
...
# ---------------------------------------------------------------- June baseline (from June CSV, actual)
def load_prior_primary():
    p = Path("/app/reports/june2026-qa-inspection/june2026_inspection_summary.csv")
    ...
    return out
JUNE_PRIMARY = load_prior_primary()
...
def w_summary():
    ...
    with (OUT/"july2026_inspection_summary.csv").open("w",newline="",encoding="utf-8") as f:
...
def w_items():
    ...
    with (OUT/"july2026_inspection_items.csv").open("w",newline="",encoding="utf-8") as f:
...
def w_store_master():
    ...
    with (OUT/"july2026_store_master.csv").open("w",newline="",encoding="utf-8") as f:
...
def build_trend_summary():
    """Jan-Jun: reuse June's published per (month,type) numbers; July computed fresh."""
    june_csv = Path("/app/reports/june2026-qa-inspection/jan_to_june2026_trend_summary.csv")
    rows = list(csv.DictReader(open(june_csv, encoding="utf-8-sig")))
    for r in rows:
    ...
    for typ in ["门店自检","QA审计","区经检查"]:
        ...
        rows.append({"month":"2026-07","inspection_type":typ,"inspection_count":len(subset),
    ...
    with (OUT/"jan_to_july2026_trend_summary.csv").open("w",newline="",encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=fields); w.writeheader()
        for r in rows: w.writerow(r)
    return rows
...
def build_inspector_trend():
    ...
    Jan-May from May bundle raw; June from June raw; July fresh."""
    agg = defaultdict(lambda: {"n":0, "scores":[]})
    postcode_by_name = {}
    # July
    for r in SUMMARY:
        k = (r["inspector_name"], r["inspection_type"], "2026-07")
        agg[k]["n"] += 1
    ...
    # June
    june_rep = {r["shopcheck_data_id"]: r for r in JUNE_R}
    for h in JUNE_H:
        if h.get("status") != 1: continue
        lc = h.get("large_category_id")
        if lc not in TYPE_MAP: continue
        typ = TYPE_MAP[lc]
        k = (h["checker_name"], typ, "2026-06")
        agg[k]["n"] += 1
        rr = june_rep.get(h["id"])
        if rr and rr.get("score") is not None:
            agg[k]["scores"].append(rr["score"])
        if rr:
            postcode_by_name.setdefault(h["checker_name"],
                                        POSTCODE_ROLE.get(rr.get("checker_post_code")) or ROLE_FALLBACK.get(typ,"Unknown"))
    # Jan-May
    ...
    with (OUT/"july2026_inspector_trend.csv").open("w",newline="",encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=fields); w.writeheader()
        for r in rows: w.writerow(r)
    return rows
...
for iid, m in MISFILED.items():
    print(f"   -> iid={iid} {m['inspector']} @ {m['store']} {m['date']} siblings={m['siblings']}")
