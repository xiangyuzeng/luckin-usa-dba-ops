...
|---|---|---|---|---|---|---|---|
| 11 | 09-15 20:47 | 21:33 | **46 min** | 6 | 20:49–21:15、21:27–21:31 | 主 `uHd_i`：指标断 20:45–20:48（4min）→ 20:47 Nodes 7→6 → **20:49–21:16 宿主存活且持续上报但不在集群内（28min）** → 21:17–21:33 硬性中断（重启，17min）→ 21:33 归队。恢复尖峰 20:49=84%（Max/60s） | **否** |
| 12 | 09-15 23:25 | 09-16 01:04 | **99 min** | 6 | 23:26–23:27 | 主 `fveZ`（**当时的 elected master**）末个数据点 23:24，此后**再未返回**；Nodes 23:29 起为 6，01:04 恢复 7；新 NodeId `iwVBdpKTSn6RTn9W_8LG1w` 首个数据点 01:05 —— **AWS 侧节点替换** | **否** |
| 13 | 09-16 06:19 | 06:32 | 13 min | 6 | 06:16、06:29–06:32 | 主 `uHd_i`：指标断 06:15–06:29（15min）→ 06:30 返回，连续 **99%/98%/84%**（Max/60s）→ 06:32 Nodes 恢复 7 | **否** |
...
- **20:45–20:48**：`uHd_i` 停止上报（4 min）；其余 6 节点零中断。20:47 Nodes 7→6（**指标先断 2 分钟，节点才离开集群**）；
- **20:49–21:16**：`uHd_i` 每分钟正常上报（4–15%），但 Nodes=6、`MasterReachableFromNode`=0，持续 **28 分钟** —— 与 09-14 数据节点 `5a-S3` 的「被踢出集群、宿主存活」完全同构，即 AWS 第 2 点所述的传输层断连；
- **21:17–21:33**：指标硬性中断 17 分钟 —— 宿主/进程重启；
- **21:33**：归队，Nodes 回到 7（21:35 CPU=37%）。

> ⚠️ 本节数据于 2026-09-17 按 **1 分钟粒度** 重核并更正。原先基于 5 分钟粒度得出的「全部 7 节点指标均无中断」不成立：`uHd_i` 的 4 分钟缺口（20:45–20:48）被标签为 20:45 的 5 分钟桶掩盖。口径与更正详见 `reports/aws-support-reply-luckycommon-20260917.md` §1–§2。
...
| 项 | 说明 |
|---|---|
| 指标来源 | CloudWatch `AWS/ES`，`get_metric_data`。**事件 11–13 时间线于 09-17 按 1 分钟粒度重核**（`CPUUtilization`/`MasterCPUUtilization` 用 `Maximum`，`Nodes`/`MasterReachableFromNode` 用 `Minimum`）；其余分析为 5 分钟粒度，节点身份区间用 1 小时粒度。2026-09-02 00:00 ~ 09-16 16:00 UTC |
...
| 🔴 粒度陷阱 | 5 分钟桶会掩盖短于一桶的指标缺口（本例 20:45–20:48 的 4 分钟缺口被标签 20:45 的桶掩盖），且 `Average`/300s 与 `Maximum`/60s 在缺口附近差异极大（61.3% vs 84%）。**对外引用节点级 CloudWatch 数值必须标注「指标名+统计量+周期」，并以 1 分钟粒度复核** |
