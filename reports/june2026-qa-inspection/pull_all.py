#!/usr/bin/env python3
"""
Pull ALL June-2026 QA-inspection raw datasets straight to raw/*.json via MCP-SSE.
READ-ONLY (SELECT only). Reuses the proven May tables / joins with the June window.
"""
import json
...
Q = {
 ...
 "june_opps": (SRV, """
   ...
   WHERE d.check_date>='2026-06-01' AND d.check_date<'2026-07-01'
...
def main():
    ...
    for name, (server, sql) in Q.items():
        ...
        print(f"[pull] {name:16s} -> {len(rows):4d} rows")
    c.stop()
    print("DONE")
