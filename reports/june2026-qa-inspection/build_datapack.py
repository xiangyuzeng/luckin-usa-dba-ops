...
# -*- coding: utf-8 -*-
"""
June 2026 QA store-inspection — DATA PACK builder.
Consumes build_core (CSVs + derived aggregates) and writes:
  june2026_qa_datapack.md    (section-keyed, human-readable — primary)
  june2026_qa_datapack.json  (same content, structured — backup)
...
cover = {
  "doc_id":"LCNA-QA-2026-006","report_period":"2026年6月",
  "date_range":"2026-06-01 .. 2026-06-30","days":30,
...
PACK["cover"]=cover
sec("[COVER/文档信息]")
p(f"- 文档编号：**LCNA-QA-2026-006**　报告期：**2026年6月**（2026-06-01 .. 2026-06-30，30 天）")
p(f"- 活跃门店：**{NPRIM} 家运营在营门店**（主巡检口径）；另有 {len(B.OPENED_NOT_INSPECTED)} 家 6/30 新开业未及巡检 → 计 {cover['active_stores_total']} 家在营门店")
p(f"  - 计数口径：t_shop_info status=1 且非测试厨房（SL12/US999xx/US00000），open_date≤2026-06-30；6/30 新开业={', '.join(disp(o['store_code']) for o in B.OPENED_NOT_INSPECTED)}")
p(f"- 巡检类型次数：门店自检 {self_insp.total()} / QA审计 {qa_auditors.total()} / 区经检查 {area_insp.total()} = **共 {len(SUMMARY)} 次**（86 次提交 − 1 误提交 = 85）")
...
biggest_s = top_s[0]
# cross-type divergence (compute here for callout c)
...
for c,pr in PRIMARY.items():
    if isinstance(pr["adjusted_total_score"],int) and isinstance(MAYP.get(c),int):
        movers.append((c, MAYP[c], pr["adjusted_total_score"], pr["adjusted_total_score"]-MAYP[c]))
improvers=sorted([m for m in movers if m[3]>0], key=lambda x:-x[3])
decliners=sorted([m for m in movers if m[3]<0], key=lambda x:x[3])

mgmt = {
 "primary_avg":prim_avg,"coverage":f"{NPRIM}/{NPRIM}=100%","may_primary_avg":85.8,"may_baseline_n":18,
 "total_findings_full_month":sum(fm_sev.values()),
 "callout_a":f"巡检量 85（5月86，−1.2%），主巡检均分 {prim_avg}（5月85.8，+{round(prim_avg-85.8,1)}），覆盖 {NPRIM}/{NPRIM}=100%；三类均分 自检/QA/区经 见 §7.5",
...
sec("[§管理摘要]")
p(f"- 主巡检均分 **{prim_avg}**（5月 85.8，**+{round(prim_avg-85.8,1)}**）；门店覆盖 **{NPRIM}/{NPRIM} = 100%**（5月基准 18 家）")
p(f"- 全月发现项合计 **{sum(fm_sev.values())}**（主巡检 {sum(pm_sev.values())}）")
p(f"- (a) 体系里程碑 vs 5月：巡检量 85（5月 86，−1 次）；主巡检均分 85.8→{prim_avg}（**+{round(prim_avg-85.8,1)}**）；覆盖率维持 100%")
p(f"- (b) 最大系统性 S 项集群：**{biggest_s[0]}** — {biggest_s[1]['n']} 项 S，涉及 {len(biggest_s[1]['stores'])} 家门店（延续 5 月 air gap / Sinks and Pipes 遗留）")
if divergences:
...
s11 = {
 ...
 "stores_below_80":sorted(below80,key=lambda x:x[1]),"coverage":f"{NPRIM}/{NPRIM}=100%","may_coverage":"18/18=100%",
}
PACK["s1_1"]=s11
sec("[§1.1] 主巡检概览")
...
p(f"- 覆盖率：**{NPRIM}/{NPRIM} = 100%**（5月 18/18 = 100%）")
...
s13={
 ...
 "improvers":[{"store":c,"may":a,"jun":b,"delta":d} for c,a,b,d in improvers],
 "decliners":[{"store":c,"may":a,"jun":b,"delta":d} for c,a,b,d in decliners],
...
p(f"- 环比改善（{len(improvers)} 家）：" + "，".join(f"{disp(c)} {a}→{b}(+{d})" for c,a,b,d in improvers))
p(f"- 环比下滑（{len(decliners)} 家）：" + "，".join(f"{disp(c)} {a}→{b}({d})" for c,a,b,d in decliners))
if improvers: p(f"- 最大变动：改善 {disp(improvers[0][0])} +{improvers[0][3]}；下滑 {disp(decliners[0][0])} {decliners[0][3]}")
...
for sv in ["S","M","G","L"]:
    main=mod_by_sev[sv].most_common(3)
...
hdr=["门店(code)"]+[B.EN2CN.get(m,m) if False else m for m in CANON]+["合计"]
rows41=[]
for c in order:
...
p(f"- 新店？{'是' if lc not in MAYP else '否'}（5月主巡检基准 {MAYP.get(lc,'—')}，环比 {lp['adjusted_total_score']-MAYP[lc] if lc in MAYP else 'n/a'}）")
PACK["s4_2"]={"store":lc,"score":lp["adjusted_total_score"],"type":lp["inspection_type"],"date":lp["inspection_date"],
              "inspector":lp["inspector_name"],"module_breakdown":dict(lmods),
              "S_items":[{"sub_item":it["sub_item"],"desc":it["description"]} for it in ls],
              "is_new":lc not in MAYP,"may_baseline":MAYP.get(lc)}
...
PACK["s4_3"]={"rows":s43,"total":len(appeals),"approved":ap_by.get("approved",0),"denied":ap_by.get("denied",0),"pending":ap_by.get("pending",0)}
...
p(f"\n空/短描述（<10字符）占比：{empty}/{attr_tot} = {empty/attr_tot*100:.1f}%")
PACK["s5_1"]={"attribution":dict(attr),"empty_short":empty,"total":attr_tot,"empty_pct":round(empty/attr_tot*100,1)}
...
for typ_ in ["门店自检","QA审计","区经检查"]:
    ...
    tempo={"门店自检":"全月高频（日常暴露）","QA审计":"月中集中（6/10–18 专业定级）","区经检查":"全月+月末复核"}[typ_]
    rows71.append([typ_,len(sub),f"{len(stores)}/{NPRIM}",len(insps),round(sum(scs)/len(scs),1) if scs else "—",S,M,tempo])
    s71[typ_]={"count":len(sub),"stores":len(stores),"inspectors":len(insps),
               "avg":round(sum(scs)/len(scs),1) if scs else None,"S":S,"M":M,"tempo":tempo}
p(mdtable(["巡检类型","次数","覆盖门店(n/N)","巡检员数","平均分","S项","M项","节奏"],rows71))
PACK["s7_1"]=s71
...
# WRITE
# ======================================================================
header=f"""# June 2026 QA 门店稽核 — 数据包 (DATA PACK)
...
"""
(HERE/"june2026_qa_datapack.md").write_text(header+"\n".join(MD), encoding="utf-8")
(HERE/"june2026_qa_datapack.json").write_text(json.dumps(PACK, ensure_ascii=False, indent=1), encoding="utf-8")
print("WROTE june2026_qa_datapack.md  + .json")
print(f"  sections in PACK: {len(PACK)}")
print(f"  Σ主巡检模块扣分 = {sum(mod_agg[m]['deduction'] for m in CANON)}  | Σ store 合计 = {sum(store_tot.values())}")
print(f"  improvers={len(improvers)} decliners={len(decliners)} divergences_ge20={len(divergences)}")
